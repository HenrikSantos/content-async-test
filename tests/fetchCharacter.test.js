...
describe('Teste a função fetchCharacter', () => {
  it('Verifica se ao passar o id 720 o nome da personagem é Wonder Woman', async () => {
    const character = await fetchCharacter('720');
    expect(character.name).toBe('Wonder Woman');
  });
  it('Verifica se ao passar o id 720 o nome da personagem não é Spider Man', async () => {
    const character = await fetchCharacter('720');
  ...
  it('Verifica se não retorna \'erro\' ao executar a função com parâmetro que não existe', async () => {
    const response = await fetchCharacter('parâmetro qualquer');
    expect(response).not.toBe('erro');
  });
